...
function ProgrammeCard({ item }) {
  ...
  const pillClass = isPaid
    ? 'relative top-2 z-10 text-xs! md:text-sm! bg-pink text-white px-2 pt-2 pb-3 rounded-t-xl font-semibold'
    : 'relative top-2 z-10 text-xs! md:text-sm! bg-blue text-white px-2 pt-2 pb-3 rounded-t-xl font-semibold';
      ...
      <div className="flip h-130 md:h-152 relative z-20 perspective">
        <div className={`flip-inner ${flipped ? 'is-flipped' : ''}`}>
          ...
          <div className="flip-face flip-back relative">
            <div className={`${backCardClass} md:mt-0 relative -top-2 z-20 flex flex-col gap-y-5 lg:gap-y-25 justify-between`}>
              <div>
